...
const App_ar: React.FC<{ onToggleLanguage: () => void }> = ({ onToggleLanguage }) => {
  ...
  const translateScenarioName = (id: string) => {
      switch(id) {
          case 'combined': return 'شاهقة ١٢ (كامل المحفظة العقارية)';
          case '1br': return 'قطاع غرفة وصالة (١٢ وحدة)';
          case '2br': return 'قطاع غرفتين وصالة (٨ وحدات)';
          case 'str': return 'قطاع قصير الأجل STR (١٣ وحدة)';
          case 'ltr': return 'قطاع طويل الأجل LTR (٧ وحدات)';
          default: return id;
      }
  };
  ...
  const translateScenarioDesc = (id: string) => {
    switch(id) {
        case 'combined': return 'تحليل شامل لمحفظة فاخرة تضم ٢٠ وحدة سكنية مدمجة. يجمع بين ١٢ شقة بغرفة نوم واحدة (٨ قصير الأجل + ٤ طويل الأجل) و ٨ شقق بغرفتي نوم (٥ قصير الأجل + ٣ طويل الأجل) لتغطية تذبذبات الطلب بين المواسم العادية والموسم القوي بالرياض.';
        case '1br': return 'تحليل مخصص لشقق غرفة وصالة الـ ١٢. تستهدف هذه الشقق الموظفين التنفيذيين والزوار لضمان عوائد متوازنة ومستدامة عالي الكفاءة.';
        case '2br': return 'دراسة مخصصة لشقق غرفتين وصالة الـ ٨ التي تجمع بين السكن التنفيذي الراق والعائلي الفخم لزيادة العوائد وتأمين إيجار مستقر.';
        case 'str': return 'المحفظة السكنية قصيرة الأجل (ستر) التي تضم ١٣ وحدة تأجيرية. تهدف بشكل رئيس للاستفادة القصوى من مواسم الفعاليات والسياحة بالرياض الشتوية.';
        case 'ltr': return 'المحفظة السكنية طويلة الأجل المستقرة التي تركز على ٧ عقود سنوية فاخرة لتفادي المخاطر التشغيلية وخفض تكاليف التسويق.';
        default: return '';
    }
  };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pb-16">
        ...
        <FadeInUp>
          <div className="text-center pt-8 pb-8 sm:pt-14 sm:pb-8">
            ...
            >
                <span className="text-[10px] sm:text-xs font-bold tracking-[0.22em] uppercase text-purple-700">مشروع شاهقة ١٢ - لوحة قياس الجدوى العقارية</span>
            </motion.div>
            <h1 className="text-4xl sm:text-7xl font-black text-[#1D1D1F] tracking-tighter mb-4 leading-none">
              شاهقة ١٢<span className="text-[#4A2C5A]">.</span>
            </h1>
            <p className="text-sm sm:text-2xl text-gray-500 max-w-3xl mx-auto font-medium leading-relaxed tracking-tight px-4 mb-8">
                تحليل العقار الفخم بالرياض في إطار <span className="text-[#4A2C5A] font-bold">النماذج الهجينة المشتركة</span>. ٢٠ وحدة سكنية متكاملة تشمل ١٢ شقة غرفة وصالة و ٨ شقق غرفتين وصالة بنوعيها قصير وطويل الأجل.
            </p>
          </div>
        </FadeInUp>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 sm:mb-12 text-right">
          <motion.div whileHover={{ y: -3 }} className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm">
            ...
            <span className="text-xl sm:text-3xl font-extrabold text-[#4A2C5A] tracking-tight">٢٠ وحدة سكنية</span>
            <span className="text-xs text-gray-500 block mt-1">١٢ غرفة وصالة | ٨ غرفتين وصالة</span>
          </motion.div>
          <motion.div whileHover={{ y: -3 }} className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">فئة قصير الأجل (STR)</span>
            <span className="text-xl sm:text-3xl font-extrabold text-amber-600 tracking-tight">١٣ وحدة سكنية</span>
            <span className="text-xs text-gray-500 block mt-1">٨ غرفة وصالة | ٥ غرفتين وصالة</span>
          </motion.div>
                    <motion.div whileHover={{ y: -3 }} className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">فئة طويل الأجل (LTR)</span>
            <span className="text-xl sm:text-3xl font-extrabold text-blue-600 tracking-tight">٧ وحدات سكنية</span>
            <span className="text-xs text-gray-500 block mt-1">٤ غرفة وصالة | ٣ غرفتين وصالة</span>
          </motion.div>
          <motion.div whileHover={{ y: -3 }} className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">توزيع الدورة النشطة</span>
            <span className="text-xl sm:text-3xl font-extrabold text-[#8A6E99] tracking-tight">٥ / ٧ أشهر</span>
            <span className="text-xs text-gray-500 block mt-1">موسم الذروة / الموسم العادي</span>
          </motion.div>
        </div>
        ...
        <div className="mb-10 text-right font-cairo">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-2">
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-500 mt-1">اختر أحد نماذج التشغيل الثلاثة الرئيسية لتشغيل كامل محفظة شاهقة ١٢ المشتركة لمعاينة الأثر المالي والفني فورياً.</p>
            </div>
          </div>
            ...
            >
              <div className="w-full">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="text-xs font-mono font-bold text-gray-400">٧ وحدات</span>
                </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between w-full">
                ...
                <span className="text-sm font-extrabold text-[#AA7C11]">١,٣٥٨,٥٠٠ ر.س</span>
              </div>
            </button>
            ...
            >
              <div className="w-full">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="text-xs font-mono font-bold text-gray-400">١٣ وحدة</span>
                </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between w-full">
                <span className="text-[10px] text-gray-400 font-mono uppercase tracking-wider">الإيرادات بمستوى الأساس</span>
                <span className="text-sm font-extrabold text-amber-600">٢,٩٧٤,٣٠٠ ر.س</span>
              </div>
            </button>
            ...
            >
              <div className="w-full">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="text-xs font-mono font-bold text-gray-400">٢٠ وحدة كاملة</span>
                </div>
                <h4 className="font-bold text-gray-900 text-base mb-1 font-cairo">النموذج الهجين المتكامل</h4>
                <p className="text-xs text-gray-500 leading-relaxed font-cairo">الخلطة الاستراتيجية: ١٣ وحدة يومي للمواسم الدافئة عوائدياً، و٧ وحدات عقود طويلة الأجل كركيزة حماية تشغيلية.</p>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between w-full">
                <span className="text-[10px] text-gray-400 font-mono uppercase tracking-wider">الإيرادات بمستوى الأساس</span>
                <span className="text-sm font-extrabold text-[#4A2C5A]">٤,٣٣٢,٨٠٠ ر.س</span>
              </div>
            </button>
          </div>
        </div>
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                         ...
                         <div className="flex items-center gap-3 text-right w-full md:w-auto">
                             ...
                             <span className="text-xs sm:text-sm font-bold text-white font-cairo">
                                 {activeModel === 'ltr' 
                                   ? "محفظة عقود طويل الأجل (٧ وحدات سكنية)" 
                                   : activeModel === 'str' 
                                     ? "محفظة الإيجار قصير الأجل (١٣ وحدات سكنية)" 
                                     : "كامل محفظة شاهقة ١٢ المشتركة (٢٠ وحدة سكنية)"}
                             </span>
                         </div>
